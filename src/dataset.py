...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, path, t=0.9, mode='all', eval_mode=False, fold=None, gt_path='labelsC.csv',
        ...
        for i in range(1, self.n_classes):
            if i in self.label_list_glob:
                self.l1_2_g[i] = self.label_list_local_1[self.label_list_glob.index(
                    i)]
                self.l2_2_g[i] = self.label_list_local_2[self.label_list_glob.index(
                    i)]

        for i in range(1, self.n_classes_local_2):
            if i in self.label_list_local_2:
                self.l1_2_l2[i] = self.label_list_local_1[self.label_list_local_2.index(
                    i)]
    ...
    def __getitem__(self, idx):
        ...
        for i in range(len(self.label_list)):
            ...
            target_local_1[target_ == self.label_list[i]
                           ] = self.label_list_local_1[i]
            target_local_2[target_ == self.label_list[i]
                           ] = self.label_list_local_2[i]
        ...
        if self.apply_cloud_masking:
            ...
            CC_mask = CC_mask.view(
                CC_mask.shape[0], 1, CC_mask.shape[1], CC_mask.shape[2])
            X = X * CC_mask.float()

        # augmentation
        if self.eval_mode is False and np.random.rand() < self.augment_rate:
