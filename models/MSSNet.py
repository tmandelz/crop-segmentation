...
class Encoder(nn.Module):
    def __init__(self, wf, scale, vscale, kernel_size, reduction, act, bias, csff):
        ...
        self.encoder_level1 = [ResBlock(
            wf,              kernel_size, reduction, bias=bias, act=act) for _ in range(2)]
        self.encoder_level2 = [ResBlock(
            wf+scale,        kernel_size, reduction, bias=bias, act=act) for _ in range(2)]
        self.encoder_level3 = [ResBlock(
            wf+scale+vscale, kernel_size, reduction, bias=bias, act=act) for _ in range(2)]
        ...
        if csff:
            self.csff_enc1 = nn.Conv2d(
                wf,              wf,              kernel_size=1, bias=bias)
            self.csff_enc2 = nn.Conv2d(
                wf+scale,        wf+scale,        kernel_size=1, bias=bias)
            self.csff_enc3 = nn.Conv2d(
                wf+scale+vscale, wf+scale+vscale, kernel_size=1, bias=bias)
...
class Decoder(nn.Module):
    def __init__(self, wf, scale, vscale, kernel_size, reduction, act, bias):
        ...
        self.decoder_level1 = [ResBlock(
            wf,              kernel_size, reduction, bias=bias, act=act) for _ in range(2)]
        self.decoder_level2 = [ResBlock(
            wf+scale,        kernel_size, reduction, bias=bias, act=act) for _ in range(2)]
        self.decoder_level3 = [ResBlock(
            wf+scale+vscale, kernel_size, reduction, bias=bias, act=act) for _ in range(2)]
        ...
        self.skip_attn1 = ResBlock(
            wf,       kernel_size, reduction, bias=bias, act=act)
        self.skip_attn2 = ResBlock(
            wf+scale, kernel_size, reduction, bias=bias, act=act)
    ...
    def forward(self, input):
        ...
        for y in range(self.downscale_factor):
            for x in range(self.downscale_factor):
                kernel[x + y * self.downscale_factor::self.downscale_factor *
                       self.downscale_factor, 0, y, x] = 1
...
class DeblurNet(nn.Module):
    def __init__(self, in_c=3, out_c=3, wf=54, scale=42, vscale=42, kernel_size=3, reduction=4, bias=False):
        ...
        self.ED_up = EDUp()
        # scale1
        self.shallow_feat1 = nn.Sequential(conv(12, wf, kernel_size, bias=bias), ResBlock(
            wf, kernel_size, reduction, bias=bias, act=act))
        self.E1_1 = Encoder(wf, scale, vscale, kernel_size,
                            reduction, act, bias, csff=False)
        self.D1_1 = Decoder(wf, scale, vscale, kernel_size,
                            reduction, act, bias)
        self.tail1_1 = Tail_shuffle(wf, 12, kernel_size, bias=bias)
        ##########################################################################
        # scale2
        self.shallow_feat2 = nn.Sequential(conv(12, wf, kernel_size, bias=bias), ResBlock(
            wf, kernel_size, reduction, bias=bias, act=act))
        self.up_scale1_feat = ScaleUpSample(wf)
        self.fusion12 = conv(wf*2, wf, kernel_size, bias=bias)

        self.E2_1 = Encoder(wf, scale, vscale, kernel_size,
                            reduction, act, bias, csff=True)
        self.D2_1 = Decoder(wf, scale, vscale, kernel_size,
                            reduction, act, bias)

        self.E2_2 = Encoder(wf, scale, vscale, kernel_size,
                            reduction, act, bias, csff=True)
        self.D2_2 = Decoder(wf, scale, vscale, kernel_size,
                            reduction, act, bias)
        ...
        ################################################################################
        # scale3
        self.shallow_feat3 = nn.Sequential(conv(3, wf, kernel_size, bias=bias), ResBlock(
            wf, kernel_size, reduction, bias=bias, act=act))
        self.up_scale2_feat = ScaleUpSample(wf)
        self.fusion23 = conv(wf*2, wf, kernel_size, bias=bias)

        self.E3_1 = Encoder(wf, scale, vscale, kernel_size,
                            reduction, act, bias, csff=True)
        self.D3_1 = Decoder(wf, scale, vscale, kernel_size,
                            reduction, act, bias)

        self.E3_2 = Encoder(wf, scale, vscale, kernel_size,
                            reduction, act, bias, csff=True)
        self.D3_2 = Decoder(wf, scale, vscale, kernel_size,
                            reduction, act, bias)

        self.E3_3 = Encoder(wf, scale, vscale, kernel_size,
                            reduction, act, bias, csff=True)
        self.D3_3 = Decoder(wf, scale, vscale, kernel_size,
                            reduction, act, bias)
